...
class ParamManager:
    ...
    def _cast(self, value: Any, name: str, tp: type[T]) -> T | None:
        ...
        if tp is bool:
            return _check_bool(value, name)  # type: ignore[reportReturnType]
        if tp is float:
            return float(value)  # type: ignore[reportReturnType]
        if tp is Path:
            return Path(value)  # type: ignore[reportReturnType]
        if get_origin(tp) is set and get_args(tp) == (str,):  # pragma: no branch
            return _extract_set_of_str(value)  # type: ignore[reportReturnType]
        if get_origin(tp) is list and get_args(tp) == (str,):
            return extract_list_of_str(value)  # type: ignore[reportReturnType]
