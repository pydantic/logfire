...
def rewrite_ast(
    ...
    logfire_args = LogfireArgs(logfire_instance._tags, logfire_instance._sample_rate)  # type: ignore[reportPrivateUsage]
...
class AutoTraceTransformer(BaseTransformer):
    ...
    def logfire_method_call_node(self, node: ast.FunctionDef | ast.AsyncFunctionDef, qualname: str) -> ast.Call:
        ...
        span_factory = partial(
            self.logfire_instance._fast_span,  # type: ignore[reportPrivateUsage]
            *self.logfire_method_arg_values(qualname, node.lineno),
        )
        if self.min_duration > 0:
            config = self.logfire_instance._config  # type: ignore[reportPrivateUsage]
        ...
        return ast.Call(
            func=ast.Subscript(
                ...
                slice=ast.Index(value=ast.Constant(value=index)),  # type: ignore[reportDeprecated]
                ctx=ast.Load(),
            ),
            args=[],
            keywords=[],
