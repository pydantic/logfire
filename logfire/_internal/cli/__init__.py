...
def _main(args: list[str] | None = None) -> None:
    ...
    parser.set_defaults(func=lambda _: parser.print_help())  # type: ignore[reportUnknownLambdaType]
    subparsers = parser.add_subparsers(title='commands', metavar='')

    # NOTE(DavidM): Let's try to keep the commands listed in alphabetical order if we can
    cmd_auth = subparsers.add_parser('auth', help=parse_auth.__doc__.split('\n', 1)[0], description=parse_auth.__doc__)  # type: ignore[reportOptionalMemberAccess]
    ...
    cmd_projects.set_defaults(func=lambda _: cmd_projects.print_help())  # type: ignore[reportUnknownLambdaType]
    ...
    cmd_read_tokens.set_defaults(func=lambda _: cmd_read_tokens.print_help())  # type: ignore[reportUnknownLambdaType]
