...
class Record:
    ...
    def from_span(cls, span: ReadableSpan) -> Record:
        ...
        return cls(
            ...
            message=attributes.get(ATTRIBUTES_MESSAGE_KEY) or span.name,  # type: ignore[reportArgumentType]
            events=span.events,
            span_id=span.context and span.context.span_id,
            parent_span_id=span.parent and span.parent.span_id,
            kind=attributes.get(ATTRIBUTES_SPAN_TYPE_KEY, 'span'),  # type: ignore[reportArgumentType]
            level=attributes.get(ATTRIBUTES_LOG_LEVEL_NUM_KEY, _INFO_LEVEL),  # type: ignore[reportArgumentType]
        )

    @classmethod
    def from_log(cls, log: LogRecord) -> Record:
        attributes = log.attributes or {}
        message: str = attributes.get(ATTRIBUTES_MESSAGE_KEY)  # type: ignore[reportAssignmentType]
        if not message:
...
class SimpleConsoleSpanExporter(SpanExporter):
    ...
    def _details_parts(self, span: Record, indent_str: str) -> TextParts:
        ...
        * the log level name
        ...
        log_level_num: int = span.attributes.get(ATTRIBUTES_LOG_LEVEL_NUM_KEY)  # type: ignore[reportAssignmentType]
    ...
    def _print_arguments(self, span: Record, indent_str: str):
        ...
        json_schema = cast('dict[str, Any]', json.loads(span.attributes.get(ATTRIBUTES_JSON_SCHEMA_KEY, '{}')))  # type: ignore[reportArgumentType]
...
def _pending_span_parent(attributes: Mapping[str, object]) -> int | None:
    ...
    if parent_id_str := attributes.get(ATTRIBUTES_PENDING_SPAN_REAL_PARENT_KEY):
        return int(parent_id_str, 16)  # type: ignore[reportArgumentType]
