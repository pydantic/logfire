...
def instrument(
    ...
    def decorator(func: Callable[P, R]) -> Callable[P, R]:
        ...
        if inspect.isgeneratorfunction(func):
            ...
            def wrapper(*func_args: P.args, **func_kwargs: P.kwargs):  # type: ignore[reportUnknownParameterType, reportRedeclaration]
            ...
            async def wrapper(*func_args: P.args, **func_kwargs: P.kwargs):  # type: ignore[reportRedeclaration]
                with open_span(*func_args, **func_kwargs):
            ...
            async def wrapper(*func_args: P.args, **func_kwargs: P.kwargs) -> R:  # type: ignore[reportRedeclaration]
        ...
        wrapper = functools.wraps(func)(wrapper)  # type: ignore[reportUnknownVariableType, reportAssignmentType]
        return wrapper
...
def get_open_span(
    ...
    final_span_name: str = span_name or attributes[ATTRIBUTES_MESSAGE_TEMPLATE_KEY]  # type: ignore[reportAssignmentType]
    ...
    def open_span(*_: P.args, **__: P.kwargs):  # type: ignore[reportRedeclaration]
        return get_logfire()._fast_span(final_span_name, attributes, **extra_span_kwargs())  # type: ignore[reportPrivateUsage]
    ...
    if extract_args is True:
        ...
        if sig.parameters:  # only extract args if there are any
            ...
            def open_span(*func_args: P.args, **func_kwargs: P.kwargs):
                ...
                return get_logfire()._instrument_span_with_args(  # type: ignore[reportPrivateUsage]
...
def get_attributes(
    ...
    if not msg_template:
        try:
            msg_template = f'Calling {inspect.getmodule(func).__name__}.{func_name}'  # type: ignore[reportOptionalMemberAccess]
    ...
    with contextlib.suppress(Exception):
        attributes.update(get_filepath_attribute(inspect.getsourcefile(func)))  # type: ignore[reportCallIssue, reportArgumentType]
