...
def _pydantic_model_encoder(o: Any, seen: set[int]) -> JsonValue:
    ...
    if isinstance(o, pydantic.RootModel):
        return to_json_value(o.root, seen)  # type: ignore[reportUnknownMemberType]

    try:
        dump = o.model_dump()
    except AttributeError:  # pragma: no cover
        # pydantic v1
        dump = o.dict()  # type: ignore[reportDeprecated]
...
def _get_sqlalchemy_data(o: Any, seen: set[int]) -> JsonValue | None:
    ...
    try:
        ...
        deferred = set()  # type: ignore[reportUnknownVariableType]
...
def to_json_value(o: Any, seen: set[int]) -> JsonValue:
    try:
        ...
        if isinstance(o, (list, tuple)):
            ...
            return [to_json_value(item, seen) for item in o]  # type: ignore[reportUnknownVariableType]
        elif isinstance(o, Mapping):
            return {
                key if isinstance(key, str) else safe_repr(key): to_json_value(value, seen)
                for key, value in o.items()  # type: ignore[reportUnknownVariableType]
            }
        ...
        try:
            ...
            to_dict = type(o).to_dict(o)  # type: ignore[reportUnknownVariableType, reportUnknownMemberType]
