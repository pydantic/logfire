...
try:
    ...
    OpenaiChatCompletionStreamState = OpenaiCompletionStreamState  # type: ignore[reportAssignmentType]
...
def on_response(
    ...
    if isinstance(response, LegacyAPIResponse):  # pragma: no cover
        on_response(response.parse(), span, version=versions)  # type: ignore[reportUnknownArgumentType]
    ...
    if isinstance(response_model := getattr(response, 'model', None), str):
        ...
        try:
            ...
            response_data = response.model_dump()  # type: ignore[reportAttributeAccessIssue, reportOptionalMemberAccess, reportUnknownMemberType, reportUnknownVariableType]
    ...
    if isinstance(response, ChatCompletion) and response.choices:
        ...
        if 1 in versions:
            try:
                events = json.loads(span.attributes['events'])  # type: ignore[reportArgumentType, reportOptionalSubscript]
            except Exception:
                pass
            else:
