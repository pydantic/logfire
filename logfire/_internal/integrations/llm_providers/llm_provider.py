...
def instrument_llm_provider(
                    ...
                    class LogfireInstrumentedAsyncStream(stream_cls):
                        async def __stream__(self) -> AsyncIterator[Any]:
                            with record_streaming(
                                ...
                                async for chunk in super().__stream__():  # type: ignore[reportUnknownMemberType, reportUnknownVariableType]
                                    record_chunk(chunk)
                                    yield chunk
                    ...
                    class LogfireInstrumentedStream(stream_cls):
                        def __stream__(self) -> Iterator[Any]:
                            with record_streaming(
                                ...
                                for chunk in super().__stream__():  # type: ignore[reportUnknownMemberType, reportUnknownVariableType]
                                    record_chunk(chunk)
                                    yield chunk
    ...
    def uninstrument_context():
        ...
        try:
            ...
            setattr(client, attr_name, client._original_request_method)  # type: ignore[reportUnknownMemberType]
...
def record_streaming(
    ...
    timer = logire_llm._config.advanced.ns_timestamp_generator  # type: ignore[reportPrivateUsage]
    start = timer()
    try:
        yield record_chunk
    finally:
