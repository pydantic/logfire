...
for _deprecated in [
    ...
    FULL_CONFIG.pop(_deprecated, None)  # type: ignore[reportArgumentType, reportCallIssue]
...
def instrument_system_metrics(logfire_instance: Logfire, config: Config | None = None, base: Base = 'basic'):
    ...
    if 'process.runtime.cpu.utilization' in config:  # type: ignore[reportUnnecessaryContains]
        ...
        del config['process.runtime.cpu.utilization']  # type: ignore[reportArgumentType]
    ...
    instrumentor = SystemMetricsInstrumentor(config=config)  # type: ignore[reportArgumentType]
