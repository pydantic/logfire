...
def patch_fastapi():
    ...
    original_solve_dependencies = fastapi.routing.solve_dependencies  # type: ignore[reportPrivateImportUsage]
    fastapi.routing.solve_dependencies = patched_solve_dependencies  # type: ignore[reportPrivateImportUsage]
        ...
        with self.logfire_instance.span('FastAPI arguments') if self.extra_spans else NoopSpan() as span:
            ...
            with handle_internal_errors:
                ...
                if isinstance(result, tuple):  # pragma: no cover
                    solved_values = result[0]  # type: ignore[reportUnknownVariableType]
                    solved_errors = result[1]  # type: ignore[reportUnknownVariableType]
                ...
                attributes: dict[str, Any] | None = {
                    ...
                    'values': {
                        k: v
                        for k, v in solved_values.items()  # type: ignore[reportUnknownMemberType, reportUnknownVariableType]
                        if not isinstance(v, (Request, WebSocket, BackgroundTasks, SecurityScopes, Response))
                    },
                    'errors': solved_errors.copy(),  # type: ignore[reportUnknownMemberType]
                }
...
class _InstrumentedValues(dict):  # type: ignore[reportMissingTypeArgument]
    request: Request
