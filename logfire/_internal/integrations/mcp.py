...
def instrument_mcp(logfire_instance: Logfire, propagate_otel_context: bool):
    ...
    original_send_request = BaseSession.send_request  # type: ignore[reportUnknownMemberType, reportUnknownVariableType]

    @functools.wraps(original_send_request)  # type: ignore[reportUnknownArgumentType]
    ...
    original_send_notification = BaseSession.send_notification  # type: ignore[reportUnknownMemberType, reportUnknownVariableType]

    @functools.wraps(original_send_notification)  # type: ignore[reportUnknownArgumentType]
    ...
    original_received_notification = ClientSession._received_notification  # type: ignore[reportPrivateUsage]
    ...
    ClientSession._received_notification = _received_notification  # type: ignore[reportPrivateUsage]

    original_handle_client_request = ClientSession._received_request  # type: ignore[reportPrivateUsage]
    ...
    ClientSession._received_request = _received_request_client  # type: ignore[reportPrivateUsage]

    original_handle_server_request = Server._handle_request  # type: ignore[reportPrivateUsage]
    ...
    Server._handle_request = _handle_request  # type: ignore[reportPrivateUsage]
        ...
        if params := getattr(root, 'params', None):
            if meta := getattr(params, 'meta', None):
                if isinstance(meta, dict):
                    dumped_meta = meta  # type: ignore[reportUnknownVariableType]
                else:
            ...
            params.meta = type(params).Meta.model_validate({**carrier, **dumped_meta})  # type: ignore[reportUnknownMemberType]
        else:
            root.params = _request_params_type_adapter(type(root)).validate_python({'_meta': carrier})  # type: ignore[reportUnknownArgumentType]
