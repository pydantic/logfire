...
class LogfireHttpxRequestInfo(RequestInfo, LogfireHttpxInfoMixin):
    ...
    def capture_body_if_form(self, attr_name: str = 'http.request.body.form') -> bool:
        ...
        data = self.form_data
        if not (data and isinstance(data, Mapping)):  # pragma: no cover  # type: ignore[reportUnnecessaryIsInstance]
            return False
    ...
    def content(self) -> bytes:
        ...
        return list(self.stream)[0]  # type: ignore[reportUnknownVariableType]

    @cached_property
    def form_data(self) -> Mapping[str, Any] | None:
        frame = inspect.currentframe().f_back.f_back.f_back  # type: ignore[reportOptionalMemberAccess]
        while frame:
    ...
    def set_complex_span_attributes(self, attributes: dict[str, Any]):
        set_user_attributes_on_raw_span(self.span, attributes)  # type: ignore[reportArgumentType]
...
class LogfireHttpxResponseInfo(ResponseInfo, LogfireHttpxInfoMixin):
    ...
    def response(self) -> httpx.Response:
        frame = inspect.currentframe().f_back.f_back  # type: ignore[reportOptionalMemberAccess]
    ...
    def capture_text_as_json(self, span: LogfireSpan, *, text: str, attr_name: str):
        ...
        span._span.set_attribute(attr_name, text)  # type: ignore[reportOptionalMemberAccess, reportPrivateUsage]
