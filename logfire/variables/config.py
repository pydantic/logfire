...
class VariableConfig(BaseModel):
        ...
        for k, labeled_value in self.labels.items():
            if labeled_value.ref is not None and labeled_value.ref != 'latest':
                if labeled_value.ref not in self.labels:
                    raise ValueError(
                        f'Label {k!r} has ref {labeled_value.ref!r} which is not present in `labels`.'
                    )
    ...
    def resolve_value(
        ...
        *,
        ...
        Returns:
            A tuple of (serialized_value, label_name, version) where:
            - serialized_value is the JSON-serialized value or None
            - label_name is the name of the label that was selected or None
        ...
        if label is not None:
            ...
            if labeled_value is not None:
                serialized, version = self._follow_ref(labeled_value)
    ...
    def _follow_ref(self, labeled_value: LabeledValue, _visited: set[str] | None = None) -> tuple[str | None, int]:
        ...
        if target is None:
            return None, labeled_value.version
        return self._follow_ref(target, _visited)
