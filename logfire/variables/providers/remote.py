...
class LogfireRemoteProvider(VariableProvider):
    ...
    def __init__(
        ...
        self._polling_interval = timedelta(seconds=polling_interval) if isinstance(polling_interval, float) else polling_interval
    ...
    def refresh(self, force: bool = False):
        ...
        with self._refresh_lock:  # Make at most one request at a time
            ...
            if not force and self._last_fetched_at is not None and self._last_fetched_at > datetime.now(tz=timezone.utc) - self._polling_interval:
                return  # nothing to do

            variables_config_data = self._get(
                '/v1/variables/', error_message='Error retrieving variables'
            )
            try:
    ...
    def shutdown(self):
        # TODO: When should this be called? If never, should we delete this code?
        #   It seems to me at least when re-configuring logfire with different VariablesOptions?
        if self._shutdown:
            return
        self._shutdown = True
