...
class ResolvedVariable(Generic[T_co]):
    ...
    Example:
        ...
        with my_var.get() as details:
            ...
            # logfire.variables.my_var = <label_name> (or '<code_default>' if no label)
            value = details.value
    ...
    def format(self, *, colors: bool = True) -> str:
        ...
        if self.errors:
            ...
            for error in self.errors:
                if error.label_name is None:  # pragma: no cover
                    lines.append(f'  {red}✗ {error.variable_name}: {error.error}{reset}')
                else:
                    lines.append(f'  {red}✗ {error.variable_name} (label: {error.label_name}){reset}')
    ...
    for variable in variables:
        ...
        if server_var is None:
            ...
            if schema_changed:
                ...
                for label_name, labeled_value in server_var.labels.items():
        ...
        for change in updates:
            ...
            if change.incompatible_labels:
                ...
                for compat in change.incompatible_labels:
                    lines.append(f'      - {compat.label_name}: {compat.error}')
...
class VariableProvider(ABC):
    ...
    def get_serialized_value_for_label(
        ...
        labeled_value = config.labels.get(label_name)
        if labeled_value is None:
            return ResolvedVariable(name=variable_name, value=None, _reason='resolved')

        serialized, version = config._follow_ref(labeled_value)
    ...
    def validate_variables(
        ...
        for var_name, label_errors in error_dict.items():
            for label_name, error in label_errors.items():
                errors.append(
