...
- **Runtime** — if an invalid composition slips through (e.g. a server value changed between validation and the next resolve), `Variable.get()` handles it in one of two ways depending on whether a usable value can still be produced:
    - **Cycles and depth overflow** (`MAX_COMPOSITION_DEPTH = 20`) abort expansion mid-graph, so there is no sensible partial value: `get()` falls back to the variable's *code default*, the resolution reason becomes `'other_error'`, and the `VariableCompositionError` is recorded on `ResolvedVariable.exception`, all with a `RuntimeWarning`.
    - **A `@{ref}@` that points at a variable which doesn't exist (or whose value isn't valid JSON)** is left *literal* in the output — e.g. `'Hello @{missing}@'` resolves to the string `'Hello @{missing}@'` rather than reverting to the code default. Keeping the value you authored, with the broken reference visible, is less surprising than silently serving a stale hardcoded default in place of your remotely-managed value. The failure is still surfaced: a `RuntimeWarning` is emitted, the `VariableCompositionError` is recorded on `ResolvedVariable.exception`, and the unresolved reference appears in `composed_from` with `error`/`error_kind` set. (This still differs from a missing `{{field}}`, which Handlebars substitutes with an empty string and is *not* treated as an error.)
