...
!!! note "An unresolved `@{ref}@` stays visible — deliberately unlike a missing `{{field}}`"
    This is an intentional difference from standard Handlebars **in interpolation position**. A missing `{{field}}` renders as an **empty string** (`{{nope}}` → ``), but an unresolved `@{ref}@` *interpolation* is left **literally in the output** — both the bare form (`@{nope}@` → `@{nope}@`) and dotted access (`@{nope.field}@` → `@{nope.field}@`).
