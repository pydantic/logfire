...
### Keep background tasks in complete traces

Ending the root span does not immediately discard a trace if another span is still running. Logfire keeps the trace in
memory until every started span ends. A running child span can still meet the sampling criteria and include the complete
trace. If no span meets the criteria, Logfire discards all the spans together after the last one ends.

After all started spans have ended, Logfire stops tracking a discarded trace to free its memory. If you then start a new
descendant of the ended root span, the new span will always be included, and its parent will be missing. This is similar
to the distributed tracing case above.

For example, a FastAPI background task may start after the root span for the request has ended and no other span is
keeping the trace active:
...
async def background_task():
    # This will be included even if tail sampling excluded the root span.
