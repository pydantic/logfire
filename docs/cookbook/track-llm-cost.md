...
## 5. Alert when daily spend crosses a threshold
...
In the sidebar, go to **Alerts** under **Notify**, click **New alert**, pick **Custom query**, and enter a query that returns a row only when today's spend exceeds your threshold (here, 50 US dollars):
