...
Make one change that *degrades* quality. Cut the agent off from its tool so it can no longer look up a real order. Disable tools for the run (a reliable switch), rather than only asking the model not to use them in the prompt, which it may not obey:

```python skip="true"
# in agent.py, add model_settings to the Agent so it can't call order_status:
support_agent = Agent(
    'google:gemini-flash-lite-latest',
    system_prompt='You are a support agent for an online store.',
    model_settings={'tool_choice': 'none'},  # the tool stays registered but can't be called
)
```
