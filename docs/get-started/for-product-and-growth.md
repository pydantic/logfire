...
2. **[Feature flags for web and mobile apps](../how-to-guides/client-side-feature-flags.md)**: serve those same flags to browser, mobile, and edge clients through the OpenFeature Remote Evaluation Protocol (OFREP), an open standard for reading feature flags from any client, so any OpenFeature-compatible app can read them without the Python SDK.
