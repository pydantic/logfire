...
To keep prices current when new models are released, install `genai-prices>=0.1.6` and enable updates:
...
This starts a background download from GitHub immediately, then repeats it once per hour. `logfire.configure()` does not wait for the first download to finish.

If your server creates worker processes, do not enable price updates before creating them. Enable price updates inside each worker instead.

The Pydantic AI, OpenAI, and Anthropic instrumentations use `genai-prices` to calculate costs. When available, the cost is attached to the span as `operation.cost` and used by the Logfire UI. This is typically more accurate because it takes into account details like cached tokens.
