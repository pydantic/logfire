...
- **Data transformation**: transform data before sending it to Logfire. For example, you can use the OpenTelemetry Collector to filter out sensitive information, extract structured data from logs or otherwise modify the data before sending it to Logfire.
    - For a detailed guide on common transformation patterns, see our guide on [Advanced Scrubbing](./scrubbing.md) with the OTel Collector.
...
See the [Alternative Backends guide](../../reference/advanced/alternative-backends.md) for more details.
...
First create a Logfire write token, see [Create Write Tokens](../../../how-to-guides/create-write-tokens.md).
