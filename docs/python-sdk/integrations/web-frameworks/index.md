...
Otherwise, check if your server uses [WSGI](../web-frameworks/wsgi.md) or [ASGI](./asgi.md) and check the corresponding integration.

You can also use [Gunicorn](./gunicorn.md) with Logfire, which is a Python WSGI HTTP server for UNIX.
...
(although usually it's better to rely on **Logfire**'s [scrubbing](../../instrumentation/scrubbing.md) feature)
