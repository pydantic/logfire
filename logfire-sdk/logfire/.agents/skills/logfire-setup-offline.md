...
With no supplied origin or trustworthy region, ask whether the account is in the US or EU; never infer it from a saved session.
...
- Never expose an ordinary Logfire write token to browser code. Direct browser export requires the restricted public token and regional base URL generated for a frontend application, passed to `configureFrontend({ baseUrl, token })` in `@pydantic/logfire-browser` 0.21.0 or later.
...
- `projects list`: Python takes `--json`; the JS CLI prints a table to stderr and ignores `--json`. If the request names an organization or project, use only that exact target; if unavailable, ask instead of substituting another. Otherwise, use the sole project when exactly one exists, ask which existing project to use when several exist, or ask for the organization and project names when none exist. Run `<target> projects new <project-name> --org <organization-name>` with the same verified prefix only in that last, zero-project case.

- Any command failing with `NonInteractiveError` explains what to do next in its own message — usually the exact missing flag, commonly `--org`. Follow what the message says and retry once. Don't drop `--non-interactive` to make the error go away; that trades a clear message for the hang it exists to prevent.
