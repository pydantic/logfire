...
**`gen_ai.usage.raw` uses the `message_start` usage object, not a synthetic merge.** *(from "Every Anthropic streaming span")*
...
This matches the non-streaming pattern established in phase 3a and the OpenAI streaming pattern from phase 2. Context: The streaming code has version branching (`SemconvVersion`) — version 1 sets `response_data`, `'latest'` sets `OUTPUT_MESSAGES`. Usage attributes are set unconditionally, outside version-specific branches.
