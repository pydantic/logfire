...
This is additive — new usage attributes are added to the dict returned by `get_attributes()`. The `response_data` dict (version 1) continues to contain `combined_chunk_content` and `chunk_count`. The `gen_ai.output.messages` (latest version) continues to contain the reconstructed assistant message. No existing attributes are modified. Existing streaming tests will gain usage attributes in their snapshots — the test mock already includes usage in `message_start` (input_tokens=25, output_tokens=25) and `message_delta` (output_tokens=55), so the streaming log span will show `gen_ai.usage.input_tokens: 25`, `gen_ai.usage.output_tokens: 55`, `gen_ai.usage.raw: {input_tokens: 25, output_tokens: 25}`, and potentially `operation.cost`. Bedrock streaming tests follow the same pattern.
