...
Today, `on_response()` has this logic inline (lines 595–609 of `openai.py`). Adding streaming would duplicate it twice more. Instead, a single shared function handles all OpenAI response objects — streaming and non-streaming, chat and Responses and embeddings. Given any OpenAI response object, the steps are always the same: get `.usage`, extract token counts via `getattr` fallback, determine `api_flavor` from the response type, call `get_usage_attributes()`. The only thing that varies is where the response object comes from, which the caller provides. Context: This works because the OpenAI SDK reconstructs full response objects from stream chunks — `OpenaiChatCompletionStreamState` produces a `ChatCompletion` via `current_completion_snapshot`, and `OpenaiResponsesStreamState` produces a `Response` via `_completed_response`. These are the same types `on_response()` handles, so the `isinstance` checks for `api_flavor` determination work unchanged. No new chunk parsing is needed.

**When usage is None, the usage attributes are simply omitted.** *(from "The usage extraction logic must not be duplicated")*
Context: For chat completions streaming, usage is only present when the caller passes `stream_options={"include_usage": True}`. For Responses API streaming, usage is always present in the `response.completed` event. The shared function returns `{}` when `usage is None`, matching the non-streaming behavior.
