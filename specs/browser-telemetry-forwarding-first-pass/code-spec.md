...
`_at_fork_reinit()` is called only in the child process after fork. It replaces the condition object, clears inherited queued requests, clears the worker reference, and installs a fresh forwarding session when called by the manager. The child does not attempt to join or close the parent worker or close the inherited parent session; only the forking thread exists in the child.
