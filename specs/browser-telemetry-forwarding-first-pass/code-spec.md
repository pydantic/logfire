...
`worker` is the non-daemon sender thread for this pipeline, or `None` before the first accepted enqueue and after shutdown has completed. Once started, the worker remains alive waiting for later work until shutdown or reconfiguration closes admission. `enqueue()` owns this lazy worker creation inline after admitting work. `closed` blocks new enqueue attempts and tells an idle worker to exit once the queue is empty. When `drain_queued=True`, already-queued memory work may still drain after `closed` becomes true. When `drain_queued=False`, shutdown explicitly drops queued work before waiting for any currently live worker. `condition` protects `queue`, `worker`, and `closed`, and is the synchronization point used by enqueue, worker wakeups, flush waits, and shutdown waits.
...
`enqueue()` starts a non-daemon worker while holding `condition` when queued work exists and no live worker is recorded. `_run()` is the worker target: it waits on the condition while the queue is empty and the pipeline is open, peeks at the first queued item, calls `_send()` inside a `try` block, removes that item from the queue in a `finally` block after the immediate send attempt completes if shutdown did not already drop it, and notifies flush/shutdown waiters when queued work changes. `_run()` exits only after `closed=True` and the memory queue is empty; it must not treat `closed=True` as a reason to abandon queued work when `drain_queued=True`. Before returning, `_run()` clears worker state so shutdown can observe that no non-daemon worker remains alive. `_send()` performs the token fanout for one queued request and delegates each Logfire-bound request to `OTLPExporterHttpSession.post()`.
