...
The worker is controlled by shutdown rather than abandoned at interpreter exit. The worker starts lazily on the first accepted enqueue, then remains alive waiting for later work until shutdown or reconfiguration closes admission. The destination session and any disk retry state remain owned by the configuration-level forwarding manager until real shutdown or reconfiguration. During shutdown, the manager waits for live workers within the caller timeout and reports incomplete if the timeout expires first.
