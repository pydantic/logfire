...
`logfire.force_flush()` and `logfire.shutdown()` include the forwarding memory queues owned by the same configuration. Flush waits for queued memory items to leave the memory queue and for any forwarding send already removed from the queue to complete its immediate `OTLPExporterHttpSession.post()` call, up to the available timeout. Shutdown stops accepting new forwarding items, drains what it can within the timeout, then waits for any in-progress forwarding send to complete even if the caller timeout has already expired, and only then shuts down forwarding-owned transport resources. A forwarding session is closed only after no active forwarding send is using it. If the caller timeout expires before all queued memory work is drained, shutdown drops any memory-queued work that has not started sending, still waits for any active send to complete, closes the now-idle session, and reports incomplete. The caller's timeout is a shared overall deadline across queued memory drain for all forwarding queues owned by the configuration; it does not permit returning while an active immediate send is still running.

When `logfire.shutdown(flush=False)` is used, forwarding shutdown still runs because forwarding owns non-daemon workers and long-lived sessions. In this mode shutdown closes admission, drops memory-queued forwarding work that has not started sending, waits for any active immediate send to complete, closes forwarding-owned sessions, and returns without treating the intentional queued-work drop as a forwarding drain timeout. It must not use `flush=False` as a reason to leave forwarding workers, sessions, or admission state alive.

**The forwarding worker is non-daemon and lifecycle-managed.** *(from "Forwarding participates in Logfire flush and shutdown")*
The worker is controlled by flush/shutdown rather than abandoned at interpreter exit. The worker may exit after draining current memory work and be recreated later, but the destination session and any disk retry state remain owned by the configuration-level forwarding manager until real shutdown or reconfiguration. During shutdown, the non-daemon worker must not be left alive after the shutdown call returns; shutdown waits for an active send to complete before returning.
