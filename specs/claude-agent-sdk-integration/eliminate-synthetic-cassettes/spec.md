...
**Defensive code for rare server-side conditions gets `# pragma: no branch`, not synthetic tests.** *(from "Real cassettes recorded from real sessions")*
...
**Add `# pragma: no branch` to the affected defensive branches.** *(from "Defensive code for rare server-side conditions", "Delete all 5 synthetic cassettes")*
All in `logfire/_internal/integrations/claude_agent_sdk.py`. The pragma goes on the `if` line, telling coverage not to require both branches:
