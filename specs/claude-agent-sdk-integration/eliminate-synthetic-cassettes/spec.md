...
**Defensive code for rare server-side conditions gets coverage pragmas, not synthetic tests.** *(from "Real cassettes recorded from real sessions")*
...
**Add coverage pragmas to the affected defensive branches.** *(from "Defensive code for rare server-side conditions", "Delete all 5 synthetic cassettes")*
All in `logfire/_internal/integrations/claude_agent_sdk.py`. Use `# pragma: no branch` when the True path is taken but the False path isn't (body executes, but the "skip" branch doesn't). Use `# pragma: no cover` when the True path is never taken (body never executes):
- `_record_result`: `if hasattr(msg, 'usage') and msg.usage:` — `no branch` (always has usage)
- `_record_result`: `if hasattr(msg, 'total_cost_usd') and msg.total_cost_usd is not None:` — `no branch` (always has cost)
- `_record_result`: `if is_error:` — `no cover` (never True in real cassettes)
- `_TurnTracker.start_turn`: `if error:` — `no cover` (never True in real cassettes)
- `_TurnTracker.close`: `if self._current_span is not None:` — `no branch` (always has a span)
