...
**Chat spans represent each assistant turn — these are the most important spans.** *(from "Spans follow OTel GenAI semantic conventions")*
Span name: `chat {model}`. One per `AssistantMessage`. Chat spans should follow the OTel GenAI semconv for the `chat` operation as closely as possible. The SDK's `AssistantMessage` provides: `content` (response blocks), `model` (string), `usage` (per-turn token usage dict with `input_tokens`, `output_tokens`, `cache_read_input_tokens`, `cache_creation_input_tokens`), and `error` (optional error type string).
...
**Chat span timing should approximate the LLM API call duration, not message-processing time.** *(from "Chat spans represent each assistant turn")*
The patched `receive_response` must handle `UserMessage` (not just `AssistantMessage` and `ResultMessage`) to track when tool results are sent back to the model. When a `UserMessage` arrives, record the current timestamp — this marks the start of the next LLM API call (the model is now thinking). When the next `AssistantMessage` triggers `start_turn`, use that recorded timestamp as the span's effective start time. For the first turn, the timestamp comes from `query()` time (already captured as `self._logfire_prompt` — capture `self._logfire_query_time` alongside it). Without this, every chat span starts when the `AssistantMessage` arrives (after the model finished responding), so the span duration measures tool execution + the *next* turn's thinking time rather than *this* turn's. This is what LangSmith's `TurnLifecycle.mark_next_start()` does.
...
It wraps the original async generator: opens an `invoke_agent` root span for the entire iteration, then inspects each yielded message. On `AssistantMessage`: closes the previous chat span (if any) and opens a new `chat {model}` sibling. On `UserMessage`: records the current timestamp for the next turn's start time. On `ResultMessage`: records usage/cost on the root span and sets error level if `is_error` is true. Each message is yielded through unchanged. If `query` was not called before `receive_response`, the prompt attribute is simply omitted.
