...
- `gen_ai.usage.partial.input_tokens` — the **total** number of input tokens for this turn, computed as `input_tokens + cache_read_input_tokens + cache_creation_input_tokens` from `AssistantMessage.usage`. The OTel GenAI semconv defines this as the total input token count; the Anthropic API's `input_tokens` field only counts uncached tokens, so we must sum all three to get the actual total. Named `partial.*` because per-turn usage from the SDK is unreliable (see below).
- `gen_ai.usage.partial.output_tokens` (from `AssistantMessage.usage`) — the SDK reports `output_tokens: 0` on intermediate `AssistantMessage`s (streamed chunks with `stop_reason: null`). The accurate total is only available on the `ResultMessage` and goes on the root `invoke_agent` span as `gen_ai.usage.output_tokens`.
- `gen_ai.usage.partial.cache_read.input_tokens`, `gen_ai.usage.partial.cache_creation.input_tokens` (from `AssistantMessage.usage`, when present — these are Anthropic-specific breakdowns)
...
Span name: `execute_tool {tool_name}`. Created by `PreToolUse` hook, ended by `PostToolUse`/`PostToolUseFailure`. Attributes: `gen_ai.operation.name` = `execute_tool`, `gen_ai.tool.name`, `gen_ai.tool.call.id` (the `tool_use_id`), `gen_ai.tool.call.arguments` (tool input), `gen_ai.tool.call.result` (tool output on success — left as-is, not converted to str, so it serializes as JSON), `error.type` (on failure). Both `PostToolUse` and `PostToolUseFailure` hooks feed results into the conversation history via `turn_tracker.add_tool_result()` so the next chat span's input messages are complete — including after tool failures. Tool results passed to `add_tool_result()` are also left as-is (not str-converted) for the same reason.
...
The SDK uses anyio internally, and anyio tasks don't propagate contextvars from the parent. We store the current parent span and logfire instance in `threading.local()` so hooks can create child spans under the correct parent. This relies on all async tasks running on the same thread — if the SDK ever used threaded execution, this approach would need revisiting.

**Context is attached only momentarily to establish parent-child relationships.** *(from "Hooks run in isolated async contexts")*
Cross-async-context `context_api.detach` always fails (the token was created in a different context), generating OTel error logs. Keeping context attached also doesn't help users — spans created inside tool functions won't become children of the tool span regardless, because the tool code runs in yet another async context. The solution: attach the root span's context, create and `_start()` the child span (which captures the parent link), then immediately detach — all in the same synchronous call. No context is left attached, so there's nothing to detach later. Both chat spans and tool spans use `_start()` / `_end()` instead of `__enter__()` / `__exit__()`.
...
`PreToolUse` creates and stores the span (keyed by `tool_use_id`). `PostToolUse`/`PostToolUseFailure` retrieves it, sets attributes, and calls `_end()`. Orphaned spans are cleaned up when the conversation ends.
