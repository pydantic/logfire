...
- **Em-dashes (`—`)**: a strong tell of unedited AI prose. Use a colon, commas, parentheses, or two sentences instead (see §5).
...
- **No em-dashes (`—`).** Use a colon for an explanation or list intro, commas or parentheses for an aside, or split into two sentences. En-dashes in ranges (`2.4–2.11`) and hyphens in compound words are fine. (Em-dashes are also a "README dialect" tell, see §4.)
