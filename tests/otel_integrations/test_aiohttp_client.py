...
@pytest.mark.anyio
async def test_aiohttp_client_capture_request_headers_only(exporter: TestExporter):
    """Test that aiohttp client captures only request headers when configured to do so."""
    ...
    # Verify that request headers are captured but response headers are not
    attributes = http_span['attributes']
    assert 'http.request.header.User-Agent' in attributes
    assert 'http.request.header.X-Custom-Header' in attributes
    # Response headers should not be captured since capture_headers only applies to requests when not using capture_all
    assert any(key.startswith('http.response.header.') for key in attributes.keys())
