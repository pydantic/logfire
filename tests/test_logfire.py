...
def test_span_with_json_schema_attribute_list_payload(exporter: TestExporter):
    """Test span creation with JSON schema attribute and list payload."""
    import json

    json_schema = {'type': 'object', 'properties': {'payload': {'type': 'object'}}}

    payload_list = ['item1', 'item2', 'item3']

    with logfire.span(
        'test_span_list',
        payload=json.dumps(payload_list),
        logfire_json_schema=json.dumps(json_schema),
    ) as s:
        pass

    # Verify individual span properties
    assert s.name == 'test_span_list'
    assert s.parent is None
    assert s.start_time is not None
    assert s.end_time is not None
    assert s.start_time < s.end_time
    ...
    assert s.attributes is not None
    assert s.attributes['payload'] == '["item1", "item2", "item3"]'
    assert s.attributes['logfire_json_schema'] == json.dumps(json_schema)
    assert s.attributes['logfire.span_type'] == 'span'
        ...
        [
            {
                ...
                'attributes': {
                    'code.filepath': 'test_logfire.py',
                    'code.lineno': 123,
                    'code.function': 'test_span_with_json_schema_attribute_list_payload',
                    'payload': '["item1", "item2", "item3"]',
                    'logfire_json_schema': json.dumps(json_schema),
                    'logfire.msg_template': 'test_span_list',
                    'logfire.msg': 'test_span_list',
                    'logfire.json_schema': '{"type":"object","properties":{"payload":{},"logfire_json_schema":{}}}',
                    'logfire.span_type': 'span',
                },
            },
        ]
...
def test_span_with_json_schema_attribute_dict_payload(exporter: TestExporter):
    """Test span creation with JSON schema attribute and dict payload."""
    import json

    json_schema = {'type': 'array', 'properties': {'payload': {'type': 'array'}}}

    payload_dict = {'id': 123, 'status': 'active'}
    ...
    # Verify attributes are set correctly
    assert s.attributes is not None
    assert s.attributes['payload'] == json.dumps(payload_dict)
    assert s.attributes['logfire_json_schema'] == json.dumps(json_schema)
    assert s.attributes['logfire.span_type'] == 'span'
        ...
        [
            {
                ...
                'attributes': {
                    'code.filepath': 'test_logfire.py',
                    'code.lineno': 123,
                    'code.function': 'test_span_with_json_schema_attribute_dict_payload',
                    'payload': json.dumps(payload_dict),
                    'logfire_json_schema': json.dumps(json_schema),
                    'logfire.msg_template': 'test_span_dict',
                    'logfire.msg': 'test_span_dict',
                    'logfire.json_schema': '{"type":"object","properties":{"payload":{},"logfire_json_schema":{}}}',
                    'logfire.span_type': 'span',
                },
            },
