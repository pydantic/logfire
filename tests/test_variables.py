...
class TestVariable:
    ...
    def test_get_creates_span_when_instrumented(
        ...
        completed_span_names = [
            s.name for s in spans if (s.attributes or {}).get('logfire.span_type') != 'pending_span'
        ]
        assert completed_span_names.index('Declare variable string_var') < completed_span_names.index(
            'Resolve variable string_var'
        )
    ...
    def test_declaration_handles_dynamic_and_bounded_metadata(
        ...
        assert large_attrs['logfire.variable.schema_preview'] == '{"description":"' + 's' * 1008
        ...
        assert large_attrs['logfire.variable.code_default_preview'] == '"' + 'x' * 1023
